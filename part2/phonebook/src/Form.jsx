function Form(props) {
    ...
    return (<form onSubmit={addPerson} className="flex flex-col bg-slate-200 p-2 py-6 items-center">
        <div className="flex w-96 mt-3">
            ...
            <input className="border border-slate-500 rounded-sm bg-slate-50 p-1 flex-1" placeholder="minimum 3 characters Required" value={newName} onChange={(e)=>{setNewName(e.target.value)}}></input>
        </div>
        <div className="flex w-96 mt-3">
            <label className="w-20">number:</label>
            <input className="border border-slate-500 rounded-sm bg-slate-50 p-1 flex-1" placeholder="use (nnn-nnnnnn) or (nn-nnnnnn) format" value={newNumber} onChange={(e)=>{setNewNumber(e.target.value)}}></input>
        </div>
